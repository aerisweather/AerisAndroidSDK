...
class LocationSearchActivity : ComponentActivity() {
    ...
    @Composable
    fun ComposeTopBar(viewModel: LocationSearchViewModel) {
        TopAppBar(
            title = {
                IconButton(
                    onClick = {
                ...
                Text(stringResource(id = R.string.activity_search), fontSize = 18.sp, color = Color.White)
            },
            ...
            actions = {
                IconButton(
                    onClick = {
                        startActivity(Intent(this@LocationSearchActivity, LocationMapActivity::class.java))
                    },
                ) {
                    Icon(
    ...
    @Composable
    private fun Render(event: SearchEvent.Success? = null, isConfirmed: Boolean? = null) {
        Scaffold(
            topBar = {
            ...
            content = {
                it
                ComposeSearch(event, isConfirmed)
            },
        )
    }

    @Composable
    private fun ComposeSearch(event: SearchEvent.Success? = null, isConfirmed: Boolean? = null) {
        Column(
            modifier = Modifier
                .fillMaxSize()
                .background(Color.Black),
    ...
    var onDlgConfirm: (myPlace: PlacesResponse?) -> Unit = {
        it?.apply {
            ...
            onViewModelEvent(SearchEvent.Confirm())
        }
    }
}
